...
- **커밋은 사람이 시킬 때만 한다.** 작업이 끝났다고 바로 커밋하지 않는다. 다 하면 무엇을 바꿨는지 보고하고 지시를 기다린다 — 확인 없이 쌓인 이력을 되돌리는 건 사람 몫이 된다. push·병합·태그·배포도 같다.
- **커밋은 동작을 확인한 뒤에 한다.** `node selfcheck.test.js` 통과는 최소 조건이지 확인이 아니다. 이 도구는 지도·드로잉·측위·사진이 **실지도에서만** 판정되므로(§2.4, README «검증 상태») dev 화면에서 사람이 눈으로 본 뒤가 커밋 시점이다.
  - 그래서 순서는 **작업 → 보고 → 사람이 dev 에서 확인 → 커밋 지시**다. 에이전트가 확인을 대신 선언하지 않는다.
  - 브라우저 없이 되는 확인(테스트, `curl` 로 키·헤더 보기)은 **먼저 다 해두고 결과를 같이 보고한다.** 사람이 화면에서 볼 것만 남기는 게 목적이다.
- **`main` 은 깨끗하게 유지한다.** 작업은 `dev` 사본에서 하고(§3.4), 라이브로 옮기는 것은 검증이 끝난 뒤 사람이 지시할 때다.
...
> **`camera=()` 는 그대로 둔다.** 지점 사진 첨부는 `<input type="file">`이 OS 선택창을 띄우는 방식이라 이 정책과 무관하다 — `capture` 속성도, `getUserMedia`도 쓰지 않는다. (`capture="environment"`를 쓰면 UA에 따라 이 헤더에 걸릴 수 있는데, 그 속성은 앨범 선택지를 없애는 부작용이 있어 채택하지 않았다. 사진 버튼은 하나다.)
...
- **라이브러리·API를 늘리면 관리 지점이 늘어난다.** 새 SKU마다 무료 한도와 일 상한을 따로 걸어야 한다. 지금은 Maps JavaScript + Geocoding + Drive 셋이며, 늘리기 전에 §2.2 사다리를 먼저 오른다.
  - **Drive API 는 과금 SKU가 아니다** — 무료 한도만 있고 넘으면 거부된다. 그래서 할당량 상한을 따로 걸지 않는다. 다만 **키 하나가 Maps(과금)와 Drive를 다 부를 수 있다**는 뜻이므로, 키를 갈라야 할 날이 오면 바꿀 곳은 `apiKey()` 호출부 한 줄이다(§5.3).
...
| 사진 DB 이름 | `geomeasure-photos` | `geomeasure-g-photos` — **반드시 달라야 함**(IndexedDB 도 origin 단위) |
| 사진 팝업 | `CustomOverlay` | 이미 있는 `HtmlOverlay` 재사용 + `transform` 으로 K 의 `yAnchor:1.25` 자리 |
| 사진 마커 배지 | 색 원+글자를 통째로 SVG | **배지가 있을 때만** SVG url + `labelOrigin` — 글자는 네이티브 `label` 유지 |
| 지도 오류 안내 자동 걷기 | 타일이 오면 무조건 걷음 | 타일 감시로 띄운 것만 걷음 — `위성+`는 키를 안 타 인증 실패에도 타일이 온다(§5.5) |
